...
# --- Constantes des Colonnes RÉSULTAT (noms dans le tableau final) ---
RES_REF           = 'Référence'
RES_DES           = 'Désignation'
RES_NUM           = 'N° BL'
...
def apply_excel_styling(ws: Worksheet, df_columns: list):
    ...
    cols_qty  = [RES_FAC_QTE, RES_BL_QTE, RES_ECART_QTE,
                 RES_FAC_REM, RES_BL_REM, RES_ECART_REM]
    ...
    for col in ws.columns:
        ...
        ws.column_dimensions[col_letter].width = min((max_length + 2) * 1.2, 50)
...
def style_recap_sheet(ws: Worksheet):
    ...
    for row in ws.iter_rows():
        ...
        if row[0].row <= 3:
            row[1].number_format = euro_format
            row[1].font = bold_font

def generer_excel(df):
    output = io.BytesIO()
    tot_fac    = df[RES_FAC_MNT].sum()
    tot_bl     = df[RES_BL_MNT].sum()
    ...
    data_recap = [
        ...
        ("", ""),
        ("Nombre de produits avec écarts constatés", mask_ecarts.sum()),
        ("Nombre d'écarts de prix", mask_prix.sum()),
        ("Nombre d'écarts de remises", mask_rem.sum()),
        ("Nombre de produits facturés non trouvés dans les BL", mask_manque_bl.sum()),
        ("Nombre de produits sur les BL non trouvés dans la facture", mask_non_fac.sum()),
    ]
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        for mask, name in [
            (mask_prix,      'Ecarts Prix'),
            (mask_rem,       'Ecarts Remise'),
            (mask_manque_bl, 'Absents BL'),
            (mask_non_fac,   'Non Facturés'),
        ]:
            if mask.sum() > 0:
        ...
        if st.button("Charger les données", type="primary"):
            try:
                ...
                if COL_BL_REF_FOURN in df_bl.columns and COL_FAC_REF_FOURN in df_fac.columns:
                    ...
                    # Normalisation remise BL : décimal (0.40) → pourcentage (40) comme la Facture
...
if st.session_state.data_loaded:
    ...
    with tab1:
        ...
        cols_view  = ["Sélectionner", COL_BL_NUM, COL_BL_REF_FOURN, COL_BL_DES,
                      COL_BL_PU, COL_BL_REM, COL_BL_QTE]
    ...
    with tab2:
        ...
        if st.session_state.manual_map:
            ...
            for k, v in st.session_state.manual_map.items():
                ...
                ca.text(f"BL [{k}] fusionné vers [{v}]")
                if cb.button("🗑️", key=f"del_{k}"): to_del = k
    ...
    with tab3:
        if st.button("🚀 Lancer le calcul", type="primary"):
            ...
            # Noms internes temporaires sans collision
            ...
            merged[RES_STATUT] = np.select(conditions,
                                           ['Manque BL', 'Non Facturé', 'Écart Prix/Remise/Qté'],
                                           default='OK')

            merged[RES_REF] = merged[FAC_REF].fillna(merged[BL_REF] + " (BL)")

            # Renommage final vers noms d'affichage UNIQUES
